...
class Gazebo_Linefollow_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        if len(road) == 0: 
            ...
            (y, x) = (center, height+road_upperbound)
            
            cv_image = cv2.circle(cv_image, (y, x), radius=radius, color=(0,0,255), thickness=-1)
        
            position = int(np.ceil(y/(width//10)))
            if position != 0:
                state[position-1] = 1
        
        text = str(state)
        cv_image = cv2.putText(cv_image,text=text, org=(10,25), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(255,255,255), thickness=2)    
